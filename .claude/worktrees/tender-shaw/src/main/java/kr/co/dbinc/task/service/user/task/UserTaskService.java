...
//        setTaskLabelNm(child);
  ...
  public RestResultVO getTasksWithChild(GetTaskListRequest request) throws Exception
  {
    RestResultVO rrVO = new RestResultVO();

    List<GetTaskDTO.GetTaskResponse> response = taskMapper.getTasks(request);
    int recordsCnt = taskMapper.getTasksCnt(request);
    ...
    for(GetTaskDTO.GetTaskResponse res : response)
    {
      ...
      //하위업무 조회
      List<GetTaskDTO.GetTaskResponse> children = taskMapper.getTasksOnlyChild(res);
