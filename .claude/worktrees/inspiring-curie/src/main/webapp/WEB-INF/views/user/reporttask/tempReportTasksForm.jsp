			...
			<main class="content">
				<div class="container-fluid p-0">
					<div style="float:right;">
						...
						<button id="btnGoAddReportTask" class="btn btn-warning mt-n1"><i class="fas fa-plus"></i> 등록</button>
					</div>
					
					<div class="mb-3">
						<h1 class="h3 d-inline align-middle"><b><i class="fas fa-list"></i> 일일업무보고 임시저장 목록</b></h1>
					</div>
						...
						<div class="col-md-12">
							<div class="card" >
								<div class="card-header">
									<h5 class="card-title" style="font-size:17px;"><i class="fas fa-fw fa-folder"></i> 일일업무보고 임시저장 목록</h5>
								</div>
								<div class="card-body">
...
function fnSetMenuSelection() {
	gfnSelectMenu("reportTaskManage", "reportTaskSide", "tempReportTasks");
}
...
function fnSetEvent() {
	...
	$("#btnGoAddReportTask").off("click").on("click", function (e) {
		e.preventDefault();
		location.href = "/user/reporttasks/reporttaskaddform";
	});
...
// 일일 업무보고 임시저장 목록 그리드 재조회
function fnReselGrid() {
	dtReportTask.ajax.reload(function (json) {
		if (json.resultCode === "SUCCESS") {
			gfnSuccessAlert(gCmmnRptTaskTmp + gCmmnSM_List, gDelay_Short);
		}
		else {
...
// 일일 업무보고 임시저장 목록 그리드 초기화 
function fnSetGrid() {
	dtReportTask = $("#taskTable").DataTable({
		ajax: {
			url : "/rest/user/reporttasks/temp/me",
            type : "POST",
			dataSrc : "data",
			data : function (d) {
