		...
		<div class="main">
			...
			<main class="content">
				<div class="container-fluid p-0">
					...
					<div class="mb-3">
						<h1 class="h3 d-inline align-middle"><i class="align-middle" data-feather="edit"></i> <b>일일업무보고 등록</b></h1>
					</div>
											...
											<form id ="frmReportTaskform">
												...
												<div class="mb-3 row">
													...
													<div id =taskReceiverMember class="col-sm-9">
														...
														<button id="btnTaskRcvMemberAdd" type="button" onclick="setTimeout(fnLoadEmpModal, 500)" class="btn btn-secondary mb-2" data-bs-toggle="modal" data-bs-target="#empAddModal"><i class="far fa-fw fa-user"></i> 추가</button>
													</div>
												</div>												
												<div class="mb-3 row">
													<label for="taskDetail" class="col-form-label col-sm-2 text-sm-end"><b>보고내용</b></label>
													<div class="col-sm-9">
														<textarea id="taskDetail" class="form-control" style="min-height: 25rem;" ></textarea>
													</div>
												</div>
											</form>
										</div>
...
function fnSetEvent() {
	...
	$("#btnTempSaveReportTask").off("click").on("click", function (e) {
		e.preventDefault();
		if(empArr.length <= 0) {
			gfnFailAlert("보고대상자는 필수 입력사항 입니다.", 5000);
	    	return false;
		}
		else if (gfnCheckRequired($("#frmReportTaskform"))) {
			fnTempSaveReportTask();
		}
	});
...
//보고 대상에 팀리더 Set
function fnSetLeader(isInit) {
	let apiUrl = "/rest/user/leader";
 	$.ajax({
		type:'get',
		url:apiUrl,
		dataType:'json',
		success: function(data) {
			if(data.empNm != null) {
				let htmlData = '';
				let buttonStyle = 'primary';
			    htmlData += '<div class="btn-group me-2" id="empAddGroup' + empHeight + '" aria-label="First group" role="group">';
			    htmlData += '<button id="btnEmpAdd' + empHeight + '" type="button" class="btn btn-'+ buttonStyle + ' mb-2">'+ data.empNm + '</button>';
			    htmlData += '<input type="text" id="empAddEmail' + empHeight + '" value="'+ data.email +'" hidden>';
			    htmlData += '<button class="btn btn-'+ buttonStyle + ' mb-2" type="button" onclick="fnDeleteEmpTask(' + empHeight + ')">';
			    htmlData += '<i class="fas fa-times"></i>';
			    htmlData += '</button>';
			    htmlData += '</div>';
...
function fnResetDeptSearch() {
	...
	$("#treeDeptList").jstree(true).search('');
}
...
function fnTempSaveReportTask() {
	...
	$.ajax({
		url:"/rest/user/reporttasks/temp",
        method:"POST",
        dataType : 'json',
		data : params,
    	beforeSend : function(xmlHttpRequest) {
		...
		if (result.resultCode == "SUCCESS") {
			...
			fnGoReportTaskUpdateForm(result.dataOne.taskId);
		}
		else {
...
function fnSendReportTask() {
	...
	$.ajax({
		type:"POST",
		url:"/rest/user/reporttasks/send",
		data: params,
		beforeSend : function(xmlHttpRequest) {
			xmlHttpRequest.setRequestHeader("AJAX", "true");
		}
...
// 상세페이지 이동
function fnGoReportTaskUpdateForm(taskId) {
	let apiUrl = '/user/reporttasks/reporttaskupdateform';
	let params = new Object();
